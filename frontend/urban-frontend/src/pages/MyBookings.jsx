...
const API = "https://urban-backend-hiv3.onrender.com"; // ✅ backend URL
...
function MyBookings() {
  ...
  const handleCancel = async (bookingId) => {
    try {
      await axios.put(`${API}/api/bookings/cancel/${bookingId}`, {});
      const updated = bookings.map((b) =>
        b._id === bookingId ? { ...b, status: "cancelled" } : b
      );
      setBookings(updated);
  ...
  const handleReviewSubmit = async () => {
    try {
      await axios.post(`${API}/api/reviews/${reviewData.bookingId}`, {
        rating: reviewData.rating,
        comment: reviewData.comment,
      });
